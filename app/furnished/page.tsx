...
import { collection, getDocs, addDoc, updateDoc, doc, query, where, serverTimestamp, Timestamp, getDoc } from 'firebase/firestore';
...
export default function FurnishedPage() {
  ...
  const loadData = async (pid: string) => {
    ...
    if (u.length > 0) setForm(f => ({ ...f, unitId: u[0].id }));
  };
  ...
  const openAdd = () => {
    setEditBooking(null);
    setForm({
      unitId: units[0]?.id || '', guestName: '', guestPhone: '',
      channel: 'airbnb', checkinDate: '', checkoutDate: '',
      totalRevenue: '', platformFee: '0', depositAmount: '0',
      depositStatus: 'held', status: 'confirmed', notes: '',
    });
    setShowModal(true);
  };
    ...
    <div dir="rtl" style={{ fontFamily: 'sans-serif', background: '#f9fafb', minHeight: '100vh' }}>
      ...
      <div style={{ padding: '16px', maxWidth: '700px', margin: '0 auto' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: '10px', marginBottom: '16px' }}>
          {[
            { label: 'صافي الإيرادات', val: totalRevenue.toLocaleString('ar-SA') + '\nر.س', color: '#16a34a', bg: '#d1fae5' },
            { label: 'الحجوزات',       val: activeBookings.length + '\nحجز نشط',             color: '#1e40af', bg: '#dbeafe' },
            { label: 'تأمين معلق',     val: pendingDeposit + '\nحجز',                        color: pendingDeposit > 0 ? '#dc2626' : '#16a34a', bg: pendingDeposit > 0 ? '#fee2e2' : '#d1fae5' },
          ].map(item => (
            <div key={item.label} style={{ background: item.bg, borderRadius: '14px', padding: '14px 12px', textAlign: 'center' }}>
              <div style={{ fontSize: '12px', color: '#6b7280', marginBottom: '6px' }}>{item.label}</div>
              {item.val.split('\n').map((line, i) => (
                <div key={i} style={{ fontSize: i === 0 ? '20px' : '11px', fontWeight: i === 0 ? '700' : '400', color: i === 0 ? item.color : '#6b7280', lineHeight: '1.2' }}>
                  {line}
                </div>
              ))}
            </div>
        ...
        {units.length > 0 && (
          <div style={{ background: '#fff', borderRadius: '16px', border: '1px solid #e5e7eb', padding: '16px', marginBottom: '16px' }}>
            ...
            {units.map(u => {
              ...
              const rev = uBookings.reduce((s, b) => s + (b.netRevenue || 0), 0);
              return (
                <div key={u.id} style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '10px' }}>
          ...
          {[['all', 'الكل'], ['confirmed', 'مؤكد'], ['checkedin', 'وصل'], ['checkedout', 'غادر'], ['cancelled', 'ملغي']].map(([v, l]) => (
            <button key={v} onClick={() => setFilterStatus(v)}
              style={{
                ...
                background: filterStatus === v ? '#1B4F72' : '#fff',
                color: filterStatus === v ? '#fff' : '#374151',
                border: filterStatus === v ? '2px solid #1B4F72' : '1px solid #e5e7eb',
              }}>
              {l}
            </button>
          ))}
        ...
        {filtered.length === 0 ? (
          <div style={{ background: '#fff', borderRadius: '16px', padding: '40px', textAlign: 'center', border: '1px solid #e5e7eb' }}>
            ...
            <button onClick={openAdd} style={{ padding: '10px 20px', background: '#1B4F72', color: '#fff', border: 'none', borderRadius: '10px', cursor: 'pointer', fontSize: '14px' }}>
              + إضافة حجز
            </button>
          </div>
        ) : (
                ...
                <div key={b.id} style={{ background: '#fff', borderRadius: '16px', border: '1px solid #e5e7eb', overflow: 'hidden', boxShadow: '0 1px 3px rgba(0,0,0,0.04)' }}>
                  ...
                  <div style={{ padding: '14px 16px' }}>
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: '12px', marginBottom: '12px' }}>
                      <div style={{ textAlign: 'center' }}>
                        <div style={{ fontSize: '11px', color: '#9ca3af', marginBottom: '4px' }}>الشقة</div>
                        <div style={{ fontSize: '18px', fontWeight: '700', color: '#1B4F72' }}>{b.unitNumber}</div>
                      </div>
                      <div style={{ textAlign: 'center' }}>
                        <div style={{ fontSize: '11px', color: '#9ca3af', marginBottom: '4px' }}>المدة</div>
                      ...
                      <div style={{ textAlign: 'center' }}>
                        <div style={{ fontSize: '11px', color: '#9ca3af', marginBottom: '4px' }}>الصافي</div>
                    ...
                    <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap', marginBottom: '12px' }}>
                      <span style={{ background: st.bg, color: st.color, padding: '3px 10px', borderRadius: '10px', fontSize: '11px', fontWeight: '500' }}>{st.label}</span>
                      {b.depositAmount > 0 && (
                        <button
                          onClick={() => b.depositStatus === 'held' && b.status === 'checkedout' ? returnDeposit(b) : undefined}
                          style={{ background: dp.bg, color: dp.color, padding: '3px 10px', borderRadius: '10px', fontSize: '11px', border: 'none', cursor: b.depositStatus === 'held' ? 'pointer' : 'default', fontWeight: '500' }}>
          ...
          <div style={{ background: '#fff', borderRadius: '20px 20px 0 0', padding: '24px', width: '100%', maxWidth: '500px', maxHeight: '90vh', overflowY: 'auto' }}
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
              ...
              <button onClick={() => setShowModal(false)}
                style={{ border: 'none', background: '#f3f4f6', borderRadius: '50%', width: '32px', height: '32px', cursor: 'pointer', fontSize: '16px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                ✕
              </button>
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
              ...
              {/* Nights indicator */}
              {nights > 0 && (
                <div style={{ background: '#dbeafe', borderRadius: '10px', padding: '10px 14px', fontSize: '14px', color: '#1e40af', fontWeight: '600', textAlign: 'center' }}>
                  {nights} ليلة
                </div>
              )}
