...
export default function DashboardPage() {
  ...
  useEffect(() => {
    const unsub = onAuthStateChanged(auth, async (fbUser) => {
      ...
      try {
        ...
        const pid = propertyId || props[0]?.id;
        if (!pid) { setLoading(false); return; }

        const selected = props.find(p => p.id === pid) || props[0];
  ...
  const pageHref = (href: string) => property ? `${href}?propertyId=${property.id}` : href;
    ...
    <div dir="rtl" style={{ fontFamily:'sans-serif', background:'#f9fafb', minHeight:'100vh' }}>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:'10px' }}>
          {properties.length > 1 && (
            <button onClick={() => router.push('/')}
              style={{ background:'rgba(255,255,255,0.15)', border:'1px solid rgba(255,255,255,0.3)', borderRadius:'10px', padding:'8px 12px', color:'#fff', cursor:'pointer', fontSize:'13px', fontFamily:'sans-serif', flexShrink:0 }}>
              ← العقارات
            </button>
          )}
          {property && (
            <div style={{ background:'rgba(255,255,255,0.1)', borderRadius:'12px', padding:'10px 14px', flex:1, display:'flex', alignItems:'center', gap:'10px' }}>
              <span style={{ fontSize:'20px' }}>🏢</span>
              <div>
                <div style={{ color:'#fff', fontSize:'15px', fontWeight:'600' }}>{property.name}</div>
                <div style={{ color:'rgba(255,255,255,0.6)', fontSize:'12px' }}>
                  {property.city && `${property.city} · `}{property.totalUnits} وحدة
                </div>
              </div>
            </div>
          )}
      ...
      <div style={{ padding:'0 16px 32px' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'12px' }}>
          {menu.map(item => (
            <a key={item.href} href={pageHref(item.href)}
