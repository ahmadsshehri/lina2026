...
function buildPaymentSchedule(tenant: Tenant, payments: Payment[]) {
  ...
  const schedule: {
    ...
    status: string; periodStart: Date; periodEnd: Date;
  }[] = [];
                      ...
                      <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                        ...
                        {canDeleteTenant && (
                          <button onClick={() => setDeleteTenantConfirm(t)}
                            style={{ padding: '9px 12px', background: '#fff', border: '1px solid #fca5a5', borderRadius: '8px', cursor: 'pointer', fontSize: '13px', color: '#dc2626' }}>
                            🗑️
                          </button>
                        )}
                        {(daysLeft < 0 || daysLeft <= 30) && (
                                ...
                                {row.status !== 'paid' && canEdit && (
                                  <div style={{ display: 'flex', gap: '6px' }}>
                                    <button onClick={() => { setShowPay({ tenant: t, period: row }); setPf(f => ({ ...f, amountDue: String(row.balance), amountPaid: String(row.balance), periodMonth: row.month, periodYear: row.year })); }}
                                      style={{ padding: '4px 10px', background: '#d1fae5', color: '#065f46', border: 'none', borderRadius: '6px', cursor: 'pointer', fontSize: '11px', fontWeight: '600' }}>سدد</button>
                                    {row.status !== 'upcoming' && (
                                      <button onClick={() => { setPartialPay({ period: row, tenant: t }); setPf(f => ({ ...f, amountDue: String(row.due), amountPaid: '', periodMonth: row.month, periodYear: row.year })); }}
                                        style={{ padding: '4px 10px', background: '#fef3c7', color: '#92400e', border: 'none', borderRadius: '6px', cursor: 'pointer', fontSize: '11px' }}>جزئي</button>
                                    )}
                                  </div>
                                )}
                              </td>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              {formerTenants.map(t => (
                <div key={t.id} style={{ background: '#fff', borderRadius: '12px', padding: '14px 16px', border: '1px solid #e5e7eb', opacity: 0.85, display: 'flex', alignItems: 'center', gap: '12px' }}>
                  ...
                  {canDeleteTenant && (
                    <button onClick={() => setDeleteTenantConfirm(t)}
                      style={{ padding: '6px 12px', background: '#fff', border: '1px solid #fca5a5', borderRadius: '8px', cursor: 'pointer', fontSize: '12px', color: '#dc2626' }}>
                      🗑️ حذف
                    </button>
                  )}
                </div>
            ...
            <div style={{ background: '#fff', borderRadius: '14px', border: '1px solid #e5e7eb', overflow: 'hidden' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12px' }}>
                <thead style={{ background: '#1B4F72' }}>
                  <tr>
                    {['الشقة', 'الاسم', 'رقم الهوية', 'الجوال', 'من', 'إلى', 'مدة الإقامة', 'مرات الاستئجار', 'الحالة', 'ملاحظات'].map(h => (
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
              ...
              <div style={{ gridColumn: '1 / -1' }}>
                ...
                <textarea value={tf.notes} onChange={e => setTf(f => ({ ...f, notes: e.target.value }))} rows={3}
                  placeholder="أي ملاحظات عن المستأجر..." style={{ ...inp, resize: 'none' }} />
              </div>
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              ...
              {pf.amountPaid && showPay.period && Number(pf.amountPaid) > showPay.period.balance && (
                <div style={{ background: '#dbeafe', borderRadius: '10px', padding: '10px 14px', fontSize: '12px', color: '#1e40af' }}>
                  💡 المبلغ الزائد سيُطبَّق على الفترات القادمة تلقائياً
                </div>
              )}
            </div>
            <div style={{ display: 'flex', gap: '10px', marginTop: '20px' }}>
        ...
        <div style={{ position: 'fixed', inset: '0', background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div style={{ background: '#fff', borderRadius: '16px', padding: '24px', width: '380px', maxWidth: '95vw' }}>
            ...
            <div style={{ display: 'flex', gap: '8px' }}>
              <button onClick={() => {
                const a = Number(pf.amountPaid);
                if (!a || a >= partialPay.period.due) { alert('أدخل مبلغاً جزئياً أقل من المستحق'); return; }
                savePeriodPayment(partialPay.tenant, partialPay.period, a, true);
              }} disabled={saving}
        ...
        <div style={{ position: 'fixed', inset: '0', background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div style={{ background: '#fff', borderRadius: '16px', padding: '24px', width: '380px', maxWidth: '95vw', textAlign: 'center' }}>
            ...
            <p style={{ color: '#6b7280', fontSize: '13px', marginBottom: '8px' }}>
              شقة {deleteTenantConfirm.unitNumber}
            </p>
            ...
            <div style={{ display: 'flex', gap: '10px' }}>
              <button onClick={() => deleteTenant(deleteTenantConfirm)} disabled={saving}
                style={{ flex: 1, padding: '12px', background: saving ? '#9ca3af' : '#dc2626', color: '#fff', border: 'none', borderRadius: '10px', cursor: 'pointer', fontSize: '14px', fontWeight: '600' }}>
