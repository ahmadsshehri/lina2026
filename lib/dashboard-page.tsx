...
const ROLE_LABEL: Record<string, string> = {
  owner: 'مالك', manager: 'مدير عقار', accountant: 'محاسب', maintenance: 'صيانة'
};
...
const MENU_ALL = [
  ...
  { label: 'المستخدمون',        sub: 'الصلاحيات والأدوار',     icon: '👥', href: '/users',     roles: ['owner'] },
];
      ...
      <div style={{ background: 'linear-gradient(135deg, #1B4F72 0%, #2E86C1 100%)', padding: '24px 20px 32px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '20px' }}>
          <div>
            ...
            <div style={{ fontSize: '22px', fontWeight: '700', color: '#fff', marginBottom: '4px' }}>{appUser?.name}</div>
          ...
          <div style={{ background: 'rgba(255,255,255,0.1)', borderRadius: '12px', padding: '12px 16px', display: 'flex', alignItems: 'center', gap: '10px' }}>
            ...
            <div>
              ...
              <div style={{ color: 'rgba(255,255,255,0.6)', fontSize: '12px' }}>
                {properties[0].city && `${properties[0].city} · `}{properties[0].totalUnits} وحدة
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
            {menu.map(item => (
              <a key={item.href} href={item.href}
                style={{ background: '#fff', borderRadius: '16px', padding: '18px 16px', textDecoration: 'none', display: 'block', border: '1px solid #f3f4f6', boxShadow: '0 1px 3px rgba(0,0,0,0.06)', position: 'relative', overflow: 'hidden' }}>
